...
class MyCoarseDropout(object):
    ...
    def __call__(self, sample):
        augmented = albu.Compose([albu.CoarseDropout(max_holes=4, max_height=20, max_width=20,
                                 ...
                                 albu.Rotate(border_mode=cv2.BORDER_CONSTANT, limit=20, p=0.8),
...
class CropCenter(object):
    ...
    def __call__(self, sample):
        ...
        if 'landmarks' in sample:
            ...
            landmarks -= torch.tensor((margin_w, margin_h),
                                      dtype=landmarks.dtype)[None, :]
            sample['landmarks'] = landmarks.reshape(-1)
...
def create_submission(path_to_data, test_predictions, path_to_submission_file):
    ...
    for i, row in mapping.iterrows():
        ...
        wf.write(file_name + ',' + ','.join(map(str,
                 needed_points.reshape(2 * len(point_index_list)))) + '\n')
